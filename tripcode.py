...
Noteworthy only in that it provides a pure-Python crypt(3) implementation,
which it uses on platforms that don't have a crypt(3) of their own (e.g.
Windows).
"""

__all__ = ['tripcode', '_crypt']
...
def _crypt(pw, salt):
    """
    Pure-Python crypt(3), used as a fallback on platforms that don't have a
    builtin crypt module.
    """
    iobuf = []
...
def tripcode(trip):
    """
    Calculates tripcode. If passed a unicode object, it will be encoded as
    SJIS first. No further input mangling is done.
    """
    global crypt

    # Convert to SJIS if and only if we received a Unicode object.
    if isinstance(trip, str):
        trip = trip.encode('sjis', 'xmlcharrefreplace')
